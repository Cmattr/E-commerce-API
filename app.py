...
# create connection
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:Richardson!629@localhost/e_commerce_db'
db = SQLAlchemy(app)
ma = Marshmallow(app)
...
def update_customer(id):
    ...
    db.session.commit()
    return jsonify({"message": "Customer detail updated successfully"}), 200
...
# app routes for customer_accounts
# GET for customer_accounts
...
def add_accounts():
    ...
    db.session.commit()
    return jsonify({"message": "New customer account created"})

# PUT for customer_accounts
...
# Get for OrderStatus by Customer id
...
# PUT to update an order status by customer
